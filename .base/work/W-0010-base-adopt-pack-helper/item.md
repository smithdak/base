...
- [x] base adopt <pack> copies a global-library pack's files into the project's .base/canon/, refusing to overwrite existing files, and prints the manual follow-ups it cannot do (INDEX.md routing lines, sync, commit)
- [x] Adoption stays a visible copy in the adopting repo's history (D-018/D-020); the helper adds zero compiler concepts
- [x] Sequencing honored: built only after the first manual adoption (W-0009, done 2026-07-16). That adoption found the copy itself low-tedium (`2026-07-16-adopt-sitecore-pack/evidence/adoption-friction.md`); Dakota directed the build anyway on 2026-07-16, superseding D-020's tedium trigger — the helper also encodes the adoption protocol so it is not re-derived from pack.md each time
